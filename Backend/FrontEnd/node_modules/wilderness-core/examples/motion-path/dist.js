...
var updatePlaybackOptions = function updatePlaybackOptions(_ref6) {
  ...
  var next = validPlaybackOptions(_extends$4({}, previous, playbackOptions, {
...
var validPlaybackOptions = function validPlaybackOptions(_ref8) {
  ...
  return _extends$4({}, playbackOptions, {
    alternate: alternate,
...
var events = function events(timeline) {
  ...
  timeline.event.previousPlaybackOptions = _extends$3({}, timeline.playbackOptions);
  timeline.event.previousState = _extends$3({}, timeline.state);
};
...
var rotatePoints = function rotatePoints(shape, angle, about) {
  return shape.map(function (point) {
    ...
    var p = _extends$7({}, point, { x: x, y: y });
    ...
    if (p.curve) {
      if (p.curve.type === 'quadratic' || p.curve.type === 'cubic') {
        ...
        p.curve = _extends$7({}, p.curve, { x1: x1, y1: y1 });
      }
      ...
      if (p.curve.type === 'cubic') {
        ...
        p.curve = _extends$7({}, p.curve, { x2: x2, y2: y2 });
      }
    }
    ...
    if (curves) {
      ...
      for (var _i3 = 0, _l3 = cubifiedPoints.length; _i3 < _l3; _i3++) {
        ...
        if (structure[_i3] && !point.curve) {
          nextPoints.push(_extends$5({}, point, {
            curve: {
              type: 'cubic',
              x1: points[_i3 - 1].x,
              y1: points[_i3 - 1].y,
  ...
  if (Array.isArray(structure[0])) {
      ...
      for (var i = 0, l = structure.length; i < l; i++) {
        if (i >= frameShape.childFrameShapes.length) {
          ...
          frameShape.childFrameShapes.push({
            ...
            points: [_extends$5({}, clone(previous[previous.length - 1]), { moveTo: true }), clone(previous[previous.length - 1])]
          });
        }
      }
    }
    ...
    for (var _i5 = 0, _l5 = structure.length; _i5 < _l5; _i5++) {
      ...
      if (!lines[_i5]) {
        ...
        lines[_i5] = [_extends$5({}, clone(previousLine[previousLine.length - 1]), { moveTo: true }), clone(previousLine[previousLine.length - 1])];
      }
  ...
  for (var i = 0, l = plainShapeObjects.length; i < l; i++) {
    ...
    if (i > 0) {
      ...
      if (delay) {
        ...
        var delayKeyframe = _extends$10({}, previousKeyframe, {
...
var arrows = {
  type: 'g',
  shapes: [{ type: 'path', d: 'M0,0L20,10L0,20Z' }, { type: 'path', d: 'M20,10L40,20L20,30Z' }, { type: 'path', d: 'M0,20L20,30L0,40Z' }, {
    type: 'g',
    shapes: [{ type: 'path', d: 'M20,10V30L0,20Z', fill: '#E54' }]
  }],
  transforms: [['scale', 0.8]]
};

var motionPathCircle = {
  type: 'circle',
  cx: 30,
  cy: 30,
  r: 30
...
var arrowsOnMotionPath = shape(arrows, _extends({}, arrows, {
  forces: [motionPath(_extends({}, motionPathCircle, {
    rotate: -90,
    accuracy: 0.1
  }))]
}));

var animation = timeline(arrowsOnMotionPath, {
  duration: 3000,
  iterations: Infinity
});

render(document.querySelector('svg'), animation, shape(_extends({}, motionPathCircle, {
  fill: 'rgba(0,0,0,0.1)',
  transforms: [['offset', 20, 20]]
})));
