...
var angle = function angle(triangle) {
  var _triangle$ = _slicedToArray$2(triangle[0], 2),
      ax = _triangle$[0],
      ay = _triangle$[1];

  var _triangle$2 = _slicedToArray$2(triangle[1], 2),
      bx = _triangle$2[0],
      by = _triangle$2[1];

  var _triangle$3 = _slicedToArray$2(triangle[2], 2),
      cx = _triangle$3[0],
      cy = _triangle$3[1];
...
var straighten = function straighten(prevPoint, point, accuracy) {
  ...
  var _curvedPoints = curvedPoints(prevPoint, point),
      _curvedPoints2 = _slicedToArray$2(_curvedPoints, 2),
...
var movePointsIndex = function movePointsIndex(shape, offset) {
  ...
  return joinLines$1([firstLine].concat(_toConsumableArray$2(restOfLines)));
};
...
var offsetPoints = function offsetPoints(shape, x, y) {
  return shape.map(function (point) {
    var p = _extends$1({}, point);

    p.x += x;
    p.y += y;

    if (p.curve) {
      p.curve = _extends$1({}, p.curve);
...
var rotatePoints = function rotatePoints(shape, angle, about) {
  return shape.map(function (point) {
    ...
    var _rotatePoint = rotatePoint(point.x, point.y, c, s, about),
        _rotatePoint2 = _slicedToArray$3(_rotatePoint, 2),
        x = _rotatePoint2[0],
        y = _rotatePoint2[1];

    var p = _extends$2({}, point, { x: x, y: y });

    if (p.curve) {
      if (p.curve.type === 'quadratic' || p.curve.type === 'cubic') {
        var _rotatePoint3 = rotatePoint(p.curve.x1, p.curve.y1, c, s, about),
            _rotatePoint4 = _slicedToArray$3(_rotatePoint3, 2),
            x1 = _rotatePoint4[0],
            y1 = _rotatePoint4[1];

        p.curve = _extends$2({}, p.curve, { x1: x1, y1: y1 });
      }

      if (p.curve.type === 'cubic') {
        var _rotatePoint5 = rotatePoint(p.curve.x2, p.curve.y2, c, s, about),
            _rotatePoint6 = _slicedToArray$3(_rotatePoint5, 2),
            x2 = _rotatePoint6[0],
            y2 = _rotatePoint6[1];

        p.curve = _extends$2({}, p.curve, { x2: x2, y2: y2 });
      }
    }
  ...
  rules.map(function (_ref) {
    ...
    if (typeof shape[prop] === 'undefined') {
      ...
      if (typeof type !== 'undefined') {
        ...
        } else if (_typeof$1(shape[prop]) !== type) {
...
var frame = function frame(timeline$$1, at) {
  if (true && ((typeof timeline$$1 === 'undefined' ? 'undefined' : _typeof(timeline$$1)) !== 'object' || !timeline$$1.timelineShapes || !timeline$$1.playbackOptions)) {
...
var frameShapeFromPlainShapeObject = function frameShapeFromPlainShapeObject(_ref) {
  var childPlainShapeObjects = _ref.shapes,
      plainShapeObject = objectWithoutProperties(_ref, ['shapes']);
  var type = plainShapeObject.type,
      ...
      attributes = objectWithoutProperties(plainShapeObject, ['type', 'height', 'width', 'x', 'y', 'cx', 'cy', 'r', 'rx', 'ry', 'x1', 'x2', 'y1', 'y2', 'd', 'points', 'shapes']);
...
var joinLines = function joinLines(lines) {
  ...
  return (_ref2 = []).concat.apply(_ref2, toConsumableArray(lines));
};
...
var tween = function tween(from, to, easing, position$$1) {
  if (typeof from === 'number') {
    ...
    if (true && to !== null && (typeof to === 'undefined' ? 'undefined' : _typeof(to)) !== 'object') {
      throw new TypeError('The tween function\'s from and to arguments must be of an identicle structure');
    }
...
var apply$2 = function apply(frameShape, _ref) {
  ...
  var transformedPoints = transformFunctions[name].apply(transformFunctions, [points].concat(toConsumableArray(args)));
...
var transformPoints = function transformPoints(points, transforms) {
  for (var i = 0, l = transforms.length; i < l; i++) {
    var _transforms$i = toArray(transforms[i]),
        name = _transforms$i[0],
        args = _transforms$i.slice(1);

    points = transformFunctions[name].apply(transformFunctions, [points].concat(toConsumableArray(args)));
  }
...
var motionPath = function motionPath(plainShapeObject) {
  ...
  var _plainShapeObject$acc = plainShapeObject.accuracy,
      ...
      coreProps = objectWithoutProperties(plainShapeObject, ['accuracy', 'easing', 'rotate', 'transforms']);
...
var keyframesAndDuration = function keyframesAndDuration(plainShapeObjects) {
  ...
  for (var i = 0, l = plainShapeObjects.length; i < l; i++) {
    var _plainShapeObjects$i = plainShapeObjects[i],
        ...
        plainShapeObject = objectWithoutProperties(_plainShapeObjects$i, ['delay', 'duration', 'easing', 'forces', 'name', 'transforms']);
...
var validProps = function validProps(_ref) {
  ...
  if (true && valid$1.apply(undefined, toConsumableArray(plainShapeObjects))) {
    if (typeof name !== 'undefined' && typeof name !== 'string' && typeof name !== 'number') {
...
exports.unitMiddleware = unitMiddleware;
